...
const Process = () => {
  const sort = 2; /* Per Page Record */
  let jobPagination = Array(Math.ceil(data.processPatient.length / sort))
    .fill()
    .map((_, i) => i + 1);

  const activePag = useRef(0);
  const jobData = useRef(
    data.processPatient.slice(activePag.current * sort, (activePag.current + 1) * sort),
  );
  ...
  const onClick = (i) => {
    ...
    jobData.current = data.processPatient.slice(
      activePag.current * sort,
      (activePag.current + 1) * sort,
    );
    setdemo(data.processPatient.slice(activePag.current * sort, (activePag.current + 1) * sort));
  };
  return (
    <Fragment>
      <div className='row'>
