...
const Pending = () => {
  const sort = 4; /* Per Page Record */
  let jobPagination = Array(Math.ceil(data.pendingPatient.length / sort))
    .fill()
    .map((_, i) => i + 1);

  const activePag = useRef(0);
  const jobData = useRef(
    data.pendingPatient.slice(activePag.current * sort, (activePag.current + 1) * sort),
  );

  const [demo, setdemo] = useState();
  const onClick = (i) => {
    activePag.current = i;
    jobData.current = data.pendingPatient.slice(
      activePag.current * sort,
      (activePag.current + 1) * sort,
    );
    setdemo(data.pendingPatient.slice(activePag.current * sort, (activePag.current + 1) * sort));
  };
  return (
    <Fragment>
      <div className='row'>
